...
## Status
Release 0.1 was the initial release. It provided UDFs for compressing and uncompressing BLOBs within the database, transparently from the application perspective.

Release 0.2 adds CRC32 checksum functions to facilitate an alternative, better scaling architecture where the application is responsible for compressing and uncompressing the data. The compressed BLOB is passed along with the CRC32 checksum of the uncompressed data. The database then validates the checksum before persisting the BLOB. BLOBs can be retrieved in compressed form for uncompression by the application.

> Note: Since the validated compressed data is compatible with release 0.1 functions, the database can still transparently uncompress the application-compressed data where appropriate.
...
## Important notes
1. In edge cases, GZIP compression can result in bigger output than input. This can happen when the input data is very short, or when the data is of a type that does not compress well. It is sensible for your designs to exempt data below a certain size from compression.
...
1. For better scaling, consider asking your developers to compress and decompress the LOB data in the application tier. This will distribute the cost across different servers:
    ...
    * If CRC32 checksums are provided with the compressed BLOB, they can be used by the database to validate that the BLOBs are GZIP compressed. The validation is cheaper than compressing the BLOB within the database.
